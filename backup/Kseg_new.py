...
def fitCurve(X, k_max, alfa, lamda, buffer):
    f = 0.5
    ...
    while k<k_max:
        ...
        indeces = findIndeces(X, eps, d, buffer)
        ...
        vertices = np.reshape(lines[:,:,0:k], (X.shape[1], 2*k), order='F')
        y, sqd = map_to_arcl(copy(edges), copy(vertices), X)
        of = np.append(of, np.array([[np.mean(sqd), np.log(np.max(y[:,0]))]]), axis = 0)
    #end while (k < k_max)
    ...
    of = np.append(of, np.array([[np.mean(sqd), np.log(np.max(y[:,0]))]]), axis = 0)
    # of = of[1:,:]
    j = np.argmin(of[:,0]+alfa*of[:,1])
    # print('Suggested number of lines: ', j+1)
