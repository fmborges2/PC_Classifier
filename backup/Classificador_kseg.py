...
def unsupervised_ksegFit(X, k_max, alfa, lamda, outlier):
    e,v = Kseg.fitCurve(X, k_max, alfa, lamda)
    y,d = Kseg.map_to_arcl(copy(e), copy(v), X)
    d = np.sort(d)[::-1]
    n = len(d)
    ...
    parametros = {'edges': e, 'vertices': v, 'limiar': limiar}  
    return parametros


def unsupervised_ksegPredict(parametros, X, Y):
    #função de predição do classificador NÃO SUPERVISIONADO
    #baseado no kseg
    #o valor retornado no score é
    #score[i] = 0 o evento é classificado como classe default e 
    #score[i] = 1 o evento é classificado como outlier
...
def supervised_ksegPredict(parametros, X):
    ...
    for i in range(nclasses):
        ...
        aux, d[:,i] = Kseg.map_to_arcl(copy(e), copy(v), X)
    #end mapping
